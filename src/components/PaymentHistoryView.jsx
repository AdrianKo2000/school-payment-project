...
const formatCurrency = (amount) => {
  ...
  // Formats numbers with standard thousands commas and adds the 'Ks' suffix
      ...
      <div className="space-y-3">
        {history.length === 0 ? (
          ...
          history.map((record) => (
            <div
              key={record.id}
              className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs hover:shadow-md transition-all flex items-center justify-between gap-4 group"
            >
              {/* Left Column: Student info and metadata block */}
              <div className="space-y-1.5 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 text-xs font-medium text-slate-500 flex-wrap">
                  <span className="flex items-center gap-1">
                    ...
                    {record.phone || "No phone added"}
                  </span>
                  <span className="flex items-center gap-1">
                    <Calendar size={13} className="text-slate-400" />
                    Cycle: {record.start_date} → {record.end_date}
                  </span>
                </div>
              </div>

              {/* Middle Column: Fees snapshot breakdown */}
              <div className="flex items-center gap-4 shrink-0 text-right">
                <div className="p-2 bg-slate-50 border border-slate-100 rounded-lg space-y-0.5">
                  <div className="text-xs font-bold text-slate-800 flex justify-between gap-4">
                    <span className="font-medium text-slate-400">School Fee:</span>
                    {formatCurrency(record.school_fee)}
                  </div>
                  <div className="text-xs font-bold text-slate-800 flex justify-between gap-4">
                    <span className="font-medium text-slate-400">Book Fee:</span>
                    {formatCurrency(record.book_fee)}
                  </div>
                </div>

                {/* Secure checkout audit timestamp badge */}
                <div className="text-right hidden sm:block">
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md mb-1">
                    <ShieldCheck size={12} /> Archived Clear
                  </span>
              ...
              {/* Right Column: Record management row destruction tool */}
              <div className="shrink-0 pl-2">
                <button
                  onClick={() => onDeleteHistoryRow(record)}
                  className="p-2 rounded-lg text-slate-300 hover:text-rose-600 hover:bg-rose-50 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all"
                  title="Permanently remove history line item"
                >
                  <Trash2 size={16} />
                </button>
              </div>
