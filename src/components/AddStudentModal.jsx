...
export default function AddStudentModal({ student, selectedClass, calculateEndDate, onAdd, onUpdate, onClose }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.start_date) errs.start_date = "Start session date is required";
      ...
      <div ref={modalRef} className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div>
            ...
            <p className="text-xs text-slate-500">Configure administrative details and billing periods.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5">Full Name *</label>
            <div className="relative">
              <User size={16} className="absolute left-3 top-3.5 text-slate-400" />
              <input type="text" value={formData.name} onChange={e => { setFormData(p=>({...p, name: e.target.value})); if(errors.name) setErrors(x=>({...x, name:null})); }} placeholder="John Doe" className={`w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm font-medium focus:outline-none transition-all ${errors.name ? 'border-rose-400 bg-rose-50/20' : 'border-slate-200 focus:border-emerald-500'}`} />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5">Start Session *</label>
              <div className="relative">
                <Calendar size={16} className="absolute left-3 top-3.5 text-slate-400" />
                <input type="date" value={formData.start_date} onChange={handleStartDateChange} className="w-full pl-9 pr-3 py-2.5 border border-slate-200 focus:border-emerald-500 rounded-xl text-sm font-semibold focus:outline-none" />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5 flex justify-between">
                End Target *
                {!isDateOverridden && formData.start_date && <span className="text-[10px] lowercase text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md border border-emerald-100 font-bold">16 weekends</span>}
              </label>
              <div className="relative">
                <Calendar size={16} className="absolute left-3 top-3.5 text-slate-400" />
                <input type="date" value={formData.end_date} onChange={handleEndDateChange} className={`w-full pl-9 pr-3 py-2.5 border rounded-xl text-sm font-semibold focus:outline-none ${isDateOverridden ? 'border-amber-300 focus:border-amber-500 bg-amber-50/10' : 'border-slate-200 focus:border-emerald-500'}`} />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5">School Tuition Fee ($) *</label>
              <div className="relative">
                <DollarSign size={16} className="absolute left-3 top-3.5 text-slate-400" />
                <input type="number" step="any" value={formData.school_fee} onChange={e => { setFormData(p=>({...p, school_fee: e.target.value})); if(errors.school_fee) setErrors(x=>({...x, school_fee:null})); }} placeholder="150.00" className="w-full pl-9 pr-4 py-2.5 border border-slate-200 focus:border-emerald-500 rounded-xl text-sm font-semibold focus:outline-none" />
              </div>
              {errors.school_fee && <p className="text-xs text-rose-600 font-medium mt-1">{errors.school_fee}</p>}
            </div>

            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-1.5">Material Book Fee ($)</label>
              <div className="relative">
                <BookOpen size={16} className="absolute left-3 top-3.5 text-slate-400" />
                <input type="number" step="any" value={formData.book_fee} onChange={e => setFormData(p=>({...p, book_fee: e.target.value}))} placeholder="45.00" className="w-full pl-9 pr-4 py-2.5 border border-slate-200 focus:border-emerald-500 rounded-xl text-sm font-semibold focus:outline-none" />
              </div>
            </div>
          </div>
          ...
          {isEditMode && (
            <div className="bg-slate-50 border p-4 rounded-xl space-y-2">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Manual State Overrides</span>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer select-none">
                  <input type="checkbox" checked={formData.tuition_paid} onChange={e => setFormData(p => ({...p, tuition_paid: e.target.checked}))} className="rounded-sm text-emerald-600 focus:ring-emerald-500 w-4 h-4" />
                  Tuition Settled
                </label>
                <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer select-none">
                  <input type="checkbox" checked={formData.book_paid} onChange={e => setFormData(p => ({...p, book_paid: e.target.checked}))} className="rounded-sm text-emerald-600 focus:ring-emerald-500 w-4 h-4" />
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2 bg-slate-50 shrink-0">
          <button type="button" onClick={onClose} disabled={submitting} className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-500 border border-slate-200 hover:bg-slate-100 bg-white rounded-lg transition-colors">Cancel</button>
          <button type="button" onClick={handleSubmit} disabled={submitting} className="flex items-center gap-2 px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold tracking-wide uppercase text-xs rounded-lg shadow-sm transition-all">
